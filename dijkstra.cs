        ...
        public int Dijkstra(int[] start, int[] goal, Node node = null)
        {
            ...
            switch (i)
            {
                case 0: //1, 3
                    ...
                    {
                        knownStates.Enqueue(new Node(newState, node.Cost + node.State[1], node.State[1], node), node.Cost + node.State[1]);
                    ...
                    if (!visited.Any(arr => arr.SequenceEqual(newState)))                 
                    {                                                                     
                        knownStates.Enqueue(new Node(newState, node.Cost + node.State[3], node.State[3], node), node.Cost + node.State[3]);
                        visited.Add(newState);                                            
                    }                                                                     
                    break;                                                                
                ...
                case 2: //1, 5                                                            
                    ...
                    if (!visited.Any(arr => arr.SequenceEqual(newState)))                 
                    {                                                                     
                        knownStates.Enqueue(new Node(newState, node.Cost + node.State[5], node.State[5], node), node.Cost + node.State[5]);
                        visited.Add(newState);                                            
                    }                                                                     
                    break;                                                                
                ...
                case 4: //1, 3, 5, 7                                                      
                    ...
                    if (!visited.Any(arr => arr.SequenceEqual(newState)))                 
                    {                                                                     
                        knownStates.Enqueue(new Node(newState, node.Cost + node.State[7], node.State[7], node), node.Cost + node.State[7]);
                        visited.Add(newState);                                            
                    }                                                                     
                    break;                                                                
            ...
            if (node.Parent == null)
                while (knownStates.Count > 0)
                {
                    ...
                    if (Dijkstra(newNode.State, goal, newNode) == 1)
                    {
                        ...
                        Node travelNode = newNode;
                        List<int> travelPath = new List<int>();
                        while (travelNode != null)
                        {
                            travelPath.Add(travelNode.Move);
                            travelNode = travelNode.Parent;
                        }

                        travelPath.Reverse();
                        Console.Write("Dijkstra Path Traveled: ");
                        foreach (int value in travelPath)
                            if (value != 0)
                                Console.Write(value + ", ");
